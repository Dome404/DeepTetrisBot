...
class TETRIS(pygame.sprite.OrderedUpdates):
    ...
    def update_current_block(self):
        try:
            self.current_block.move_down_auto(self)
        except BottomReached:
            self.stop_moving_current_block()
            self._pick_new_block()
        else:
    ...
    def move_current_block(self):
        ...
        if self._current_block_movement_heading is None:
            return
        action = {
            pygame.K_DOWN: self.current_block.move_down_press,
...
def main():
    ...
    next_block_text = font.render(
        "Next figure:", True, WHITE, bgcolor)
    score_msg_text = font.render(
        "Score:", True, WHITE, bgcolor)
    game_over_text = font.render(
        "Game over!", True, YELLOW1, bgcolor)
    ...
    while run:
        for event in pygame.event.get():
            ...
            if event.type == pygame.QUIT:
                ...
                if not paused and not game_over:
                    if event.key in MOVEMENT_KEYS:
                        tetris.stop_moving_current_block()
                    elif event.key == pygame.K_UP:
                        tetris.rotate_current_block()
            ...
            if event.type == pygame.KEYDOWN:
                if event.key in MOVEMENT_KEYS:
                    tetris.start_moving_current_block(event.key)
            # Movement Handling
            try:
                if event.type == EVENT_UPDATE_CURRENT_BLOCK:
                    tetris.update_current_block()
                elif event.type == EVENT_MOVE_CURRENT_BLOCK:
                    tetris.move_current_block()
            except TopReached:
                game_over = True
